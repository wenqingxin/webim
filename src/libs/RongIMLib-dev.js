...
(function (RongIMLib) {
    var RongIMClient = (function () {
        ...
        RongIMClient.init = function (appKey, dataAccessProvider, options) {
            ...
            //部署到http服务器，需要固定为https，因为以http访问融云，会得到动态ip作为消息服务地址，https访问得到的是域名的消息服务地址，由于公司网络限制不好开通访问的外部地址，需要固定访问地址，所以采用https来访问融云
            protocol = "https://";
            wsScheme = "wss://";
         ...
         * @param userId 第一次连接该参数是undefined(modified by chenhj)
         */
        RongIMClient.connect = function (token, callback, userId) {
            ...
            //_dataAccessProvider（WebSQLDataProvider或者ServerDataProvider的实例）的connect方法没有第三个参数userId， modified by chenhj
            //RongIMClient._dataAccessProvider.connect(token, callback, userId);
            RongIMClient._dataAccessProvider.connect(token, callback);
        };
        ...
        RongIMClient.prototype.sendTextMessage = function (conversationType, targetId, content, sendMessageCallback) {
            console.log("sendTextMessage content....");
            console.log(content);
...
(function (RongIMLib) {
    ...
    var Channel = (function () {
        function Channel(address, cb, self) {
            ...
            //self->Client
            this.self = self;
            //socket.socket->SocketTransportation
...
(function (RongIMLib) {
    var SocketTransportation = (function () {
        ...
        SocketTransportation.prototype.createTransport = function (url, method) {
            ...
            console.log(RongIMLib.RongIMClient._memoryStore.depend.wsScheme + url);
    ...
    var DBUtil = (function () {
        ...
        DBUtil.prototype.init = function (userId) {
            ...
            if (me.db) {
                ...
                var converUniSql = "CREATE UNIQUE INDEX if not exists UNI_conversation_" + userId + " ON t_conversation_" + userId + "(conversationType, targetId)";
                ...
                console.log('sql-->'+converSql);
                me.execUpdate(converSql);
                console.log('sql-->'+converUniSql);
                me.execUpdate(converUniSql);
                console.log('sql-->'+messageSql);
        ...
        DBUtil.prototype.execUpdateByParamsToCallback = function (sql, values, callback) {
            console.log("sql->"+sql);
            console.log(values);
            this.db.transaction(function (tx) {
                tx.executeSql(sql, values,
                ...
                if (callback && callback.onError) {
                    callback.onError(tx.code, tx.message);
                }
            });
        };
        ...
        DBUtil.prototype.execUpdateToCallback = function (sql, callback) {
            this.db.transaction(function (tx) {
                tx.executeSql(sql, function(tx, results){
                    if (callback && callback.onSuccess) {
                        callback.onSuccess(results.rows, results.rowsAffected);
...
(function (RongIMLib) {
    var WebSQLDataProvider = (function () {
        ...
        WebSQLDataProvider.prototype.sendMessage = function (conversationType, targetId, messageContent, sendCallback, mentiondMsg, pushText, appData, methodType) {
            ...
            msg.messageType = messageContent.messageName;
            //added by chenhj
            msg._sentTime= msg.sentTime+' '+ Math.floor(Math.random()*1000);
            sendCallback && sendCallback.onPrepare && sendCallback.onPrepare(msg);
            RongIMLib.RongIMClient.bridge.pubMsg(conversationType.valueOf(), content, targetId, {
                onSuccess: function (data) {
                    data && data.timestamp && RongIMLib.RongIMClient._storageProvider.setItem('converST_' + RongIMLib.Bridge._client.userId + conversationType + targetId, data.timestamp);
                    if ((conversationType == RongIMLib.ConversationType.DISCUSSION || conversationType == RongIMLib.ConversationType.GROUP) && messageContent.messageName == RongIMLib.RongIMClient.MessageType["ReadReceiptRequestMessage"]) {
        ...
        WebSQLDataProvider.prototype.addConversation = function (conver, callback) {
            ...
            //     //modified by chenhj 执行查询时rowsAffected一直是0
            ...
            //     callback.onSuccess(true);
            // });
            var iSql = "REPLACE INTO t_conversation_" + me.database.userId + "(conversationType,targetId,content,sentTime,isTop) values(?,?,?,?,?)";
            me.database.execUpdateByParams(iSql, [conver.conversationType, conver.targetId, JSON.stringify(conver), conver.sentTime, conver.isTop]);
            callback.onSuccess(true);
        };
        WebSQLDataProvider.prototype.removeConversation = function (conversationType, targetId, callback) {
            ...
            console.log('param: '+conversationType+' '+targetId)
            console.log(sql);
        ...
        WebSQLDataProvider.prototype.getConversation = function (conversationType, targetId, callback) {
            var sql = "select t.content, t.isTop from t_conversation_" + this.database.userId + " t where t.conversationType = ? and t.targetId = ?", conver = null;
        ...
        WebSQLDataProvider.prototype.getConversationList = function (callback, conversationTypes, count, isHidden) {
            ...
            //查询置顶会话。
            var tSql = "select * from t_conversation_" + this.database.userId + " t where t.isTop = 'true' ";
            //排序查询会话。
            var oSql = "select * from t_conversation_" + this.database.userId + " c where c.isTop != 'true' ";
            var me = this;
            if (conversationTypes) {
            ...
            tSql += " union " + oSql;
            //会话去重并选取最新的会话 added by wenqingxin
            //tSql = "select t2.conversationType,t2.targetId,t2.content,max(t2.sentTime),t2.isTop from ("+tSql+") t2 group by t2.targetId order by t2.sentTime";
            //排序按置顶标志和发送时间正序排（如果界面渲染顺序有调整注意排序字段加上desc）
            tSql = "select t2.conversationType,t2.targetId,t2.content,t2.sentTime,t2.isTop from ("+tSql+") t2 order by (case when t2.isTop='true' then 1 else 0 end) , t2.sentTime  ";
            console.log('sql----------------------------:'+tSql)
            this.database.execSearch(tSql, function (results) {
                ...
                if (results.length > 0) {
                    for (var i = 0, len = results.length; i < len; i++) {
                        ...
                        if (content) {
                            content.isTop = results[i].isTop;
                        }
                        convers.push(content);
                    }
                }
                console.log(convers[0]);
                console.log(convers[convers.length-1]);
        ...
        WebSQLDataProvider.prototype.addMessage = function (conversationType, targetId, message, callback) {
            ...
            if (callback) {
                ...
                this.database.execSearchByParams(searchSql, [message.sentTime, conversationType, targetId], function (results, rowsAffected) {
                    ...
                    results.length > 0 && (message.messageId = results[0].id);
                    callback.onSuccess(message);
                });
            }
        };
        ...
        WebSQLDataProvider.prototype.getHistoryMessages = function (conversationType, targetId, timestrap, count, callback) {
            ...
            me.database.execSearchByParams(sql, params, function (result, rowsAffected) {
                ...
                if (len < count) {
                    ...
                    RongIMLib.RongIMClient._memoryStore.lastReadTime.set(conversationType + targetId, result[0].sentTime);//modify by wenqingxin
                }
            });
        };
        ...
        WebSQLDataProvider.prototype.setConversationToTop = function (conversationType, targetId, isTop, callback) {
            ...
            // callback.onSuccess(true);
            this.database.execUpdateByParamsToCallback(sql, [isTop, conversationType , targetId], {
                ...
                onError : function (code, message) {
                    console.log("会话置顶失败");
                    throw new Error("{errorCode:" + code + ",content:" + message + "}");
                }
            });
        ...
        WebSQLDataProvider.prototype.setMessageExtra = function (messageUId, value, callback) {
            ...
            this.database.execSearchByParams(sSql, [messageUId], function (results, rowsAffected) {
                //modified by chenhj，执行查询时!rowsAffected一直是真
                //if (results.length == 0 && !rowsAffected) {
                if (results.length == 0) {
                    callback.onSuccess(false);
                }
                    ...
                    var msg = JSON.parse(results[0].content);
                    msg.extra = value;
                    _this.database.execUpdateByParams(uSql, [JSON.stringify(msg), messageUId]);
                }
            });
        };
